...
fn main() {
    ...
    for (u, v, t) in uvt{
        ...
        graph[v - 1].push((u - 1, t));
    }
    for i in 0..n{
        graph[i].push((warp, x[i]));
        graph[warp].push((i, x[i] + y));
    }
    let mut dist = vec![INF as usize; n + 1];
    let mut heap = BinaryHeap::new();
    dist[0] = 0 as usize;
    heap.push(Reverse((0, 0)));
    while let Some(Reverse((d, now))) = heap.pop(){
        if d != dist[now]{
            continue;
        }
        for &(new, w) in &graph[now]{
            ...
            if new_d < dist[new]{
                dist[new] = new_d;
                heap.push(Reverse((new_d, new)));
            }
        }
    }
    dist.remove(warp);
    dist.remove(0);
    println!("{}", dist.iter().map(|n| n.to_string()).collect::<Vec<_>>().join(" "));
}