...
fn main() {
    input! {
        rt: isize, ct: isize, ra: isize, ca: isize,
    ...
    for i in 0..m{
        sa_sum += sa[i].1;
        changed.insert(sa_sum);
    }
    for i in 0..l{
        tb_sum += tb[i].1;
        changed.insert(tb_sum);
    }
    let mut changed_vec = changed.iter().collect::<Vec<_>>();
    let mut diff: (i128, i128) = ((rt as i128) - (ra as i128), (ct as i128) - (ca as i128));
    let mut ans: i128 = 0;
    let mut s_step = vec![sa[0].0.clone()];
    let mut t_step = vec![tb[0].0.clone()];
    let mut s_n = 0;
    let mut t_n = 0;
    let mut s_count = sa[0].1;
    let mut t_count = tb[0].1;

    for i in 0..changed_vec.len(){
        let t = changed_vec[i];

        while s_n < m && s_count <= *t {
            s_n += 1;
            if s_n < m { s_count += sa[s_n].1; }
        }
        while t_n < l && t_count <= *t {
            t_n += 1;
            if t_n < l { t_count += tb[t_n].1; }
        }

        if s_n < m {
            s_step.push(sa[s_n].0.clone());
        }
        if t_n < l {
            t_step.push(tb[t_n].0.clone());
        }
    }

    let mut lens: Vec<i128> = Vec::with_capacity(changed_vec.len());
    let mut prev: usize = 0;
    for &t in &changed_vec {
        lens.push((*t - prev) as i128);
        prev = *t;
    }

    let mut to_vec = |s: &String| -> (i128, i128) {
        match s.as_bytes()[0] {
            b'U' => (-1, 0),
            b'D' => ( 1, 0),
            b'L' => ( 0,-1),
    ...
    // 相対速度だけ見ていく
    for i in 0..segs {
        let (sr, sc) = to_vec(&s_step[i]);
        let (tr, tc) = to_vec(&t_step[i]);
        let rvx = sr - tr;
        let rvy = sc - tc;
        let len = lens[i];
        ...
        if rvx == 0 && rvy == 0 {
            ...
            let mut k_opt: Option<i128> = None;
            ...
            if boolean {
                if rvy == 0 {
                    ...
                    if num % rvy != 0{ 
                        ...
                        let ky = num / rvy;
                        if let Some(kx) = k_opt {
                            if kx != ky { boolean = false; }
                        }
                        k_opt = Some(ky);
                    }
                }
